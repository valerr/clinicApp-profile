    ...
    return (
      ...
      <section className="modal-main rounded">
        <div className="m-3 pl-3">
          ...
          <button type="button" onClick={hideModal} className="close">x</button><br/>
          <div className="d-flex justify-content-around mt-5">
            <button onClick={() => {
              handleCancel(modal.id)
              setModal({type: null})
              }} className="btn btn-primary" type="submit">Да</button>
            ...
            <p className="card-text">{item.address}</p>
              <div className="d-flex justify-content-between">
                <div className="d-flex flex-row">
                  ...
                  <button onClick={() => setModal({type: 'remove', id: item.id})} className="btn btn-violet h-25 my-auto">Отменить</button>
              </div>
        </div>
      </div>
      )
